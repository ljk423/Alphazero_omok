...
class Omok:
    ...
    def clone(self):
        st = Omok()
        st.turn_monitor = self.turn_monitor
...
class Node:
    def __init__(self, move=None, parent=None, state=None):
        ...
        self.wins = 0
        self.visits = 0
        self.untriedMoves = list(legal_moves_generator(state.board, state.turn_monitor).keys())  # future child nodes
        self.playerJustMoved = 3-state.turn_monitor

    def UCTselect_child(self):
...
def UCT(rootstate, itermax, model, verbose=False):
    ...
    for i in range(itermax):
        ...
        while node.untriedMoves == [] and node.childNodes != []:
            node = node.UCTselect_child()
            state.move(state.turn_monitor, node.move)
            depth = 1

        # Expand
        if node.untriedMoves != []:
            m = random.choice(node.untriedMoves)
            state.move(state.turn_monitor, m)
            node = node.AddChild(m, state)

        # Rollout
        while state.game_status == "In Progress" and depth <= 5:
            state.move(state.turn_monitor, random.choice(list((legal_moves_generator(state.board, turn_monitor).keys()))))
            depth += 1

        # Backpropagate
        while node != None:
            node.update(scoring_model(model, state.board))
...
def scoring_model(model, current_board_state):
    score = model.predict(current_board_state.reshape(1, 15, 15, 1))
    return score
...
def train(model, print_progress=False):
    ...
    while(1):
        if game.game_status() == "In Progress" and game.turn_monitor == 1:
            game_status, board = game.move(game.turn_monitor, UCT(rootstate=game, itermax=100, model=model, verbose=False))
            scores_list.append(scoring_model(model, board))
            new_board_states_list.append(board)
            ...
            game2.board = oppo_board
            game2.turn_monitor = 3 - game.turn_monitor

            selected_move = UCT(rootstate=game2, itermax=100, model=model, verbose=False)
